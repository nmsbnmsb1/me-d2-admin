<template>
	<d2-container>
		<el-alert title="由于演示功能特殊，请注意在需要时刷新您的浏览器（以重置路由设置）查看效果" type="warning" show-icon />
...
export default {
	...
	methods: {
		...
		onClick({ title, name, path, component }) {
					...
					children: [
						{
							path,
							name,
							component: () => import('@/views_system/demo/playground/add-routes/alternates/' + component + '.vue'),
							meta: {
								title,
							},
						},
			...
			// 更新标签页池
			this.pageInit([...Vue.prototype.$frameInRoutes, ...route]);
			// 演示更新菜单
			const menuGroup = {
				title: '临时菜单',
				icon: 'plus-square',
