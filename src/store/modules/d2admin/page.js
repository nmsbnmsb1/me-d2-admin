	...
	actions: {
		...
		async openedLoad({ state, commit, dispatch, rootState }) {
			...
			const value = await dispatch(
				'd2admin/db/get',
				{
					...
					defaultValue: () =>
						role_id
							? [{ name: `${Constants.Roles[role_id].key}.index`, fullPath: `/${Constants.Roles[role_id].key}/index`, meta: { title: '首页' } }]
							: setting.page.opened,
					user: true,
				},
				{ root: true }
		...
		async close({ state, commit, dispatch }, { tagName }) {
			...
			if (isCurrent) {
				...
				await Vue.prototype.$routerInstance.push(routerObj);
			}
		},
		/**
		 * @class opened
		...
		async closeLeft({ state, commit, dispatch, rootState }, { pageSelect } = {}) {
			...
			if (currentIndex > 0) {
				...
				for (let i = state.opened.length - 1; i >= 0; i--) {
					...
					if (key && state.opened[i].name === `${key}.index`) continue;
					//
			...
			if (Vue.prototype.$routerInstance.app.$route.fullPath !== pageAim) await Vue.prototype.$routerInstance.push(pageAim);
		},
		/**
		 * @class opened
		...
		async closeAll({ state, commit, dispatch, rootState }, includeIndex = false) {
			...
			if (!includeIndex && Vue.prototype.$routerInstance.app.$route.name !== `${key}.index`) {
				//console.log(Vue.prototype.$routerInstance.app.$route.name);
				await Vue.prototype.$routerInstance.push({ name: `${key}.index` });
			}
		},
	},
	mutations: {
