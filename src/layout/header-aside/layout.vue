<template>
	<div class="d2-layout-header-aside-group" :style="styleLayoutMainGroup" :class="{ grayMode: grayActive }">
		...
		<div class="d2-layout-header-aside-content" flex="dir:top">
			<!-- 顶栏 -->
			<div class="d2-theme-header" :style="{ opacity: this.searchActive ? 0.5 : 1 }" flex-box="0" flex="main:justify cross:center">
				<router-link
					to="/index"
					...
					flex-box="0"
				>
					<img :src="logo" style="width: 44px; height: 44px; margin-left: 5px" />
					<span v-if="!asideCollapse" style="margin-left: 7px" class="text-title">{{ title }}</span>
				</router-link>
...
export default {
	...
	data() {
		return {
			...
			logo: 'image/theme/d2/logo/icon-only.png',
		};
	},
	computed: {
		...mapState('d2admin', {
