...
export function DEMO_MOCK_LIST() {
	// 模拟数据
	mock.onAny('/demo/business/issues/142/fetch').reply(...tools.responseSuccess({ list: db }));
	// 接口请求
...
export function DEMO_MOCK_DETAIL(id) {
	// 模拟数据
	mock.onAny('/demo/business/issues/142/detail').reply((config) => tools.responseSuccess(find(db, { id: config.params.id })));
	// 接口请求
	return requestForMock({
